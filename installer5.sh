...
printf "fuck you hanayo api and pep.py api"
printf " Ignore : "
...
askPrerequisites(){
	...
	printf "\n\n..:: BANCHO SERVER ::.."
	printf "\ncikey [YOUR_BANCHO_KEY_HERE]: "
	read peppy_cikey
	peppy_cikey=${peppy_cikey:=YOUR_BANCHO_KEY_HERE}
...
install-redis-and-nginx(){
	...
	sed -i 's#include ~/ripple/nginx/*.conf\*#include '$MasterDir'/nginx/*.conf#' /etc/nginx/nginx.conf
	cd $MasterDir
	cd nginx
	...
	wget -O old-frontend.conf https://raw.githubusercontent.com/keepsobp/old-frontend/master/old-frontend.conf
...
setup-hanayo(){
	...
	mv ~/go/bin/hanayo ./
	mv ~/go/src/zxq.co/ripple/hanayo/data ./data
	mv ~/go/src/zxq.co/ripple/hanayo/scripts ./scripts
	mv ~/go/src/zxq.co/ripple/hanayo/static ./static
	mv ~/go/src/zxq.co/ripple/hanayo/templates ./templates
	mv ~/go/src/zxq.co/ripple/hanayo/website-docs ./website-docs
...
setup-api(){
	...
	#Ugly fix?
	mv ~/go/src/zxq.co/ripple ~/ripple
	mv ~/go/src/zxq.co/rippleapi ~/go/src/zxq.co/ripple
...
setup-avatar-server(){
	...
	mv ~/go/bin/avatar-server-go ./avatar-server/avatar-server
